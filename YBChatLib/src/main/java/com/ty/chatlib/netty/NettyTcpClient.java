...
import com.ty.chatlib.ExecutorServiceFactory;
import com.ty.chatlib.handler.HeartbeatHandler;
import com.ty.chatlib.IMSConfig;
import com.ty.chatlib.MsgDispatcher;
import com.ty.chatlib.MsgTimeoutTimerManager;
...
import com.ty.chatlib.protocol.KfOnline;
...
public class NettyTcpClient implements IMSClientInterface {
    ...
    public void resetConnect(boolean isFirst) {
        if (!isFirst) {
            SystemClock.sleep(IMSConfig.DEFAULT_RECONNECT_INTERVAL);
        }
     ...
     */
    @Override
    public void sendMsg(GeneratedMessageV3 msg, boolean isJoinTimeoutManager) {
        if (msg == null) {
            LogUtils.d("发送消息失败，消息为空\tmessage=" + msg);
            return;
        }
        ...
        if (channel == null) {
            LogUtils.d("发送消息失败，channel为空\tmessage=" + msg);
        }

        try {
            channel.writeAndFlush(msg);
        } catch (Exception ex) {
            LogUtils.d("发送消息失败，reason:" + ex.getMessage() + "\tmessage=" + msg);
        }
    }
    ...
    @Override
    public KfMessage getHandshakeMsg() {
        KfOnline kfOnline = KfOnline.newBuilder()
                .setFromId(AA.userId)
                .setAppkey(AA.appKey)
                .setAppRequestSource(AA.source)
                .setAppSig(AA.appSig)
                .build();
    ...
    @Override
    public KfMessage getHeartbeatMsg() {
    ...
    private void onConnectStatusCallback(int connectStatus) {
        ...
        switch (connectStatus) {
            ...
            case IMSConfig.CONNECT_STATE_SUCCESSFUL: {
                LogUtils.d(String.format("Tcp连接成功，host『%s』, port『%s』", currentHost, currentPort));
                if (mIMSConnectStatusCallback != null) {
                    mIMSConnectStatusCallback.onConnected();
                }
                // 连接成功，发送握手消息
                KfMessage handshakeMsg = getHandshakeMsg();
                if (handshakeMsg != null) {
                    LogUtils.d("连接成功 发送注册登录： \n message：" + handshakeMsg);
                    sendMsg(handshakeMsg, false);
                } else {
                    LogUtils.d("请应用层构建注册登录消息！");
                }
                break;
            }
    ...
    public void addHeartbeatHandler() {
        ...
        try {
            ...
            e.printStackTrace();
            LogUtils.d("添加心跳消息管理handler失败，reason：" + e.getMessage());
        }
    }
    ...
    private void removeHandler(String handlerName) {
        try {
            ...
            e.printStackTrace();
            LogUtils.d("移除handler失败，handlerName=" + handlerName);
        }
    }
    ...
    private void closeChannel() {
        try {
            ...
            e.printStackTrace();
            LogUtils.d("关闭channel出错，reason:" + e.getMessage());
        } finally {
            channel = null;
        }
    }
    ...
    private void toServer() {
        try {
            ...
            LogUtils.d(String.format("连接Server(ip[%s], port[%s])失败", currentHost, currentPort));
            channel = null;
        }
    }
        ...
        private int connectServer() {
            ...
            for (int i = 0; (!isClosed && i < serverUrlList.size()); i++) {
                ...
                for (int j = 1; j <= IMSConfig.DEFAULT_RECONNECT_COUNT; j++) {
                    ...
                    LogUtils.d(String.format("正在进行『%s』的第『%d』次连接，当前重连延时时长为『%dms』", serverUrl, j, j * getReconnectInterval()));
